...
| **Self-Regulating AI** · 30/01/2026 – 10/02/2026 | Homeostatic regulator adjusting learning rate from the error signal, plus a meta-learning failure-detection pipeline (KS test for drift, logistic regression) that predicts breakdowns before they cascade. Modular: entropy monitoring, drift detection, decision engine. Docker + GitHub Actions CI. | Python 3.9+, scikit-learn, Random Forest, NumPy, Docker | **4 methods compared** on 5D linear regression under induced distribution shift. **Per method — the regulator: pre-shift MSE 0.486, post-shift peak 3.915. Fixed LR: 0.435 / 3.829. Step decay: 1.039 / 6.542. Adam-style: 2.243 / 4.527.** So a FIXED learning rate beat the regulator on BOTH error measures; the regulator won on learning-rate stability only. Never quote 3.829 or 0.435 as his result — they are the baseline's. The honest claim is "most stable LR behaviour, second on error". Repo wording: "near-best pre-shift performance while maintaining the most stable LR behavior post-perturbation". **Limits the repo states itself:** linear agent only (NN untested), no cosine-annealing/warm-restart comparison, alpha needs tuning, no convergence guarantees. |
